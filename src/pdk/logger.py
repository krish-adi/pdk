...
from pdk.settings import process
...
class Logger:
    ...
    def _click_echo(self, msg: Union[str, dict], event: str):
        ...
        if 'info' in event:
            click.echo(f'{click.style("PDK::INFO:   ", fg="green")}{msg}')
        elif 'warning' in event:
            click.echo(
                f'{click.style("PDK::WARN:   ", fg="yellow")}{msg}')
        elif 'error' in event:
            click.echo(f'{click.style("PDK::ERROR:  ", fg="red")}{msg}')
        elif 'test' in event:
            click.echo(f'{click.style("PDK::TEST:   ", fg="cyan")}{msg}')
        elif 'dev' in event:
            click.echo(f'{click.style("PDK::DEV:    ", fg="yellow")}\n{msg}')
        else:
            click.echo(f'{click.style("PDK::INFO:   ", fg="cyan")}{msg}')
